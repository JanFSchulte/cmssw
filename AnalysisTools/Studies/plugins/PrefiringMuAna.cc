...
namespace {
  ...
  struct MuonInfo {
    ...
    void reset(){
      ...
      tag_p4 = probe_p4 = LorentzVector();
      tag_q = probe_q = 0;
      mass = probe_dxy = probe_dz = 0;
...
class PrefiringMuAna : public edm::one::EDAnalyzer<edm::one::SharedResources,edm::one::WatchRuns,edm::one::WatchLuminosityBlocks>  {
public:
  explicit PrefiringMuAna(const edm::ParameterSet&);
  ~PrefiringMuAna();
...
void PrefiringMuAna::trigger_init(const edm::TriggerNames & triggerNames){
  trigger_ids_.clear();
...
bool PrefiringMuAna::triggered(const reco::Muon& muon){
...
PrefiringMuAna::PrefiringMuAna(const edm::ParameterSet& iConfig):
  ...
  hltPrescaleProvider_(iConfig, consumesCollector(), *this)
{
  ...
  muonTree_->Branch("tag_p4",                &muon_.tag_p4);
  muonTree_->Branch("tag_q",                 &muon_.tag_q);
  ...
  muonTree_->Branch("mass",                  &muon_.mass);
...
void PrefiringMuAna::beginRun(edm::Run const& iRun, edm::EventSetup const& iSetup) {
  bool changed(true);
  hltPrescaleProvider_.init(iRun, iSetup, "HLT", changed);
}

void PrefiringMuAna::beginLuminosityBlock(const edm::LuminosityBlock& lumiBlock, const edm::EventSetup& setup)
{
...
void PrefiringMuAna::endLuminosityBlock(edm::LuminosityBlock const& lumiBlock, const edm::EventSetup& setup)
{
  lumiTree_->Fill();
}

PrefiringMuAna::~PrefiringMuAna()
{
}

bool PrefiringMuAna::isTagMuon(const reco::Muon& muon){
...
bool PrefiringMuAna::isGoodMuon(const reco::Muon& muon){
...
bool PrefiringMuAna::isLooseMuon(const reco::Muon& muon,const reco::Vertex& vtx){
  if (not muon::isLooseMuon(muon)) return false;
  return true;
}
bool PrefiringMuAna::isMediumMuon(const reco::Muon& muon,const reco::Vertex& vtx){
  if (not muon::isMediumMuon(muon)) return false;
  return true;
}
bool PrefiringMuAna::isTightMuon(const reco::Muon& muon,const reco::Vertex& vtx){
  if (not muon::isTightMuon(muon,vtx)) return false;
  return true;
}
bool PrefiringMuAna::isIsolatedMuon(const reco::Muon& muon){
  if ( muon.isolationR03().sumPt / muon.pt() > 0.1 ) return false;
  return true;
}
bool PrefiringMuAna::isIsolatedMuonTight(const reco::Muon& muon){
  if ( muon.isolationR03().sumPt / muon.pt() > 0.05 ) return false;
  return true;
}
bool PrefiringMuAna::isPFIsolatedMuonTight(const reco::Muon& muon){
...
bool PrefiringMuAna::isPFIsolatedMuonLoose(const reco::Muon& muon){
...
std::optional<GlobalPoint> 
PrefiringMuAna::getMuonDirection(const reco::MuonChamberMatch& chamberMatch,
				 const edm::ESHandle<GlobalTrackingGeometry>& geometry,
				 const DetId& chamberId) {
...
std::vector<L1ObjInfo>
PrefiringMuAna::findL1Objects(const reco::Muon& muon,
			      const edm::ESHandle<GlobalTrackingGeometry>& geometry) 
{
...
std::vector<L1TFObjInfo>
PrefiringMuAna::findL1ObjectsByTrackFinder(const reco::Muon& muon,
			      const edm::ESHandle<GlobalTrackingGeometry>& geometry) 
{
...
void
PrefiringMuAna::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
  lumi_.nEvents++;
  ...
  for ( unsigned int i=0; i!=muonHandle->size(); ++i){
    const auto& tag_muon = muonHandle->at(i);
    if ( not isTagMuon(tag_muon) ) continue;
    for ( unsigned int j=0; j!=muonHandle->size(); ++j){
      if ( i==j ) continue;
      const auto& probe_muon = muonHandle->at(j);
      //if ( not isGoodMuon(probe_muon) ) continue;

      // Z-mass restriction
      // if ( fabs( (tag_muon.p4()+probe_muon.p4()).mass()-91.2 ) > 5 ) continue;

      // get L1 object
      
      // the candidate collection is actually indexed by BX, accessed like this:
      // for (int ibx = l1Handle->getFirstBX(); ibx <= l1Handle->getLastBX(); ++ibx) {
      // 	std::cout << "BX: " << ibx << std::endl;
      // 	for (auto it = l1Handle->begin(ibx); it != l1Handle->end(ibx); it++){
      // 	  std::cout <<
      // 	    "\t\tPt = " << it->pt() <<
      // 	    "\teta = "  << it->eta() <<
      // 	    "\tphi = "  << it->phi() <<
      // 	    "\tquality = " << it->hwQual() << std::endl;
      // 	}
      // }
      
      //if (probe_muon.pt()<10) continue;
      
      muon_.reset();
      muon_.n_vertex = vertex_count;
      muon_.tag_p4   = tag_muon.p4();
      muon_.tag_q    = tag_muon.charge();
      muon_.probe_p4 = probe_muon.p4();
      muon_.probe_q  = probe_muon.charge();
      muon_.mass     = (tag_muon.p4()+probe_muon.p4()).mass();
	...
	muon_.probe_trackerLayers = 999.;
      ...
      //  std::cout << "Tag Pt = " << tag_muon.pt() << "\tEta = " << tag_muon.eta() << "\tPhi = " << tag_muon.phi() << "\n";
      
      if (not l1muons.empty()){
	// std::cout << "Offline Pt = " << probe_muon.pt() << "\tEta = " << probe_muon.eta() << "\tPhi = " << probe_muon.phi() << "\n";
	for ( const auto& l1: l1muons ){
	  // std::cout << "\tBX = " << l1.bx <<
	  //   "\tL1Pt = " << l1.obj.pt() <<
	  //   "\teta = "  << l1.obj.eta() <<
	  //   "\tphi = "  << l1.obj.phi() <<
	  //   "\tquality = " << l1.obj.hwQual() << std::endl;
...
void PrefiringMuAna::analyzeTrigger(const edm::Event& iEvent,
				    const edm::EventSetup& iSetup,
				    const std::string& triggerName) {
  using namespace std;
  using namespace edm;
...
void
PrefiringMuAna::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(PrefiringMuAna);
