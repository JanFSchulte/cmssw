...
class Run3ScoutingParticleToRecoPFCandidateWithTrackProducer : public edm::stream::EDProducer<> {
public:
  explicit Run3ScoutingParticleToRecoPFCandidateWithTrackProducer(const edm::ParameterSet &);
  ~Run3ScoutingParticleToRecoPFCandidateWithTrackProducer() override;
  ...
  int matchCandidateToTrack(Run3ScoutingParticle cand, edm::Handle<reco::TrackCollection> trackHandle);
  void clearVars();

private:
  const edm::EDGetTokenT<std::vector<Run3ScoutingParticle>> input_scoutingparticle_token_;
  const edm::EDGetTokenT<reco::TrackCollection> input_track_token_;
...
//
Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::Run3ScoutingParticleToRecoPFCandidateWithTrackProducer(
    const edm::ParameterSet &iConfig)
    : input_scoutingparticle_token_(consumes(iConfig.getParameter<edm::InputTag>("scoutingparticle"))),
      input_track_token_(consumes(iConfig.getParameter<edm::InputTag>("tracks"))),
...
Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::~Run3ScoutingParticleToRecoPFCandidateWithTrackProducer() = default;




int Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::matchCandidateToTrack(Run3ScoutingParticle cand, edm::Handle<reco::TrackCollection> trackHandle){
  ...
  for (unsigned int itrack = 0; itrack < trackHandle->size(); ++itrack) {
    ...
    auto scoutingTrack = (*trackHandle)[itrack];
    float pt = cand.trk_pt();
    float eta = cand.trk_eta();
    float phi = cand.trk_phi();
    if (cand.relative_trk_vars()){
       pt += cand.pt();
       eta += cand.eta();
       phi += cand.phi();
    }

    if (is_close(pt, scoutingTrack.pt(), 0.000001)
        && is_close(eta, scoutingTrack.eta(), 0.000001)
        && deltaPhi(phi, scoutingTrack.phi()) <= 0.000001
    ) {
        match = itrack;
    }
...
reco::PFCandidate Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::createPFCand(Run3ScoutingParticle scoutingparticle) {
  ...
  auto q = pdTable_->particle(HepPDT::ParticleID(scoutingparticle.pdgId())) != nullptr
               ...
               : -99.f;
  if (m < -90 or q < -90) {
    LogDebug("createPFCand") << "<Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::createPFCand>:" << std::endl
...
void Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::createPFCandidates(
  ...
  for (unsigned int icand = 0; icand < scoutingparticleHandle->size(); ++icand) {
    ...
    int trackIndex = matchCandidateToTrack(scoutingparticle, trackHandle);
    if (trackIndex >= 0 and pfcand.particleId() < 4){
	reco::TrackRef trackRef(trackHandle, trackIndex);
...
void Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::createPFCandidatesSK(
    edm::Handle<std::vector<Run3ScoutingParticle>> scoutingparticleHandle,
    edm::Handle<reco::TrackCollection> trackHandle,
    std::unique_ptr<reco::PFCandidateCollection> &pfcands) {
  ...
  for (auto iter = scoutingparticleHandle->begin(),
    ...
    auto m = pdTable_->particle(HepPDT::ParticleID(iter->pdgId())) != nullptr
                 ...
                 : -99.f;
    if (m < -90) {
      LogDebug("createPFCandidatesSK") << "<Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::createPFCandidatesSK>:"
                                       << std::endl
                                       << "Unrecognisable pdgId - skipping particle" << std::endl;
      continue;
    }
  ...
  for (auto &particle : soft_killed_particles) {
    ...
    auto pfcand = createPFCand(scoutingparticle);
    for (unsigned int itrack = 0; itrack < trackHandle->size(); ++itrack){
	auto &track = (*trackHandle)[itrack];
	if (scoutingparticle.relative_trk_vars()){
	    if (scoutingparticle.trk_pt() + scoutingparticle.pt() == track.pt() && scoutingparticle.trk_eta() + scoutingparticle.eta() == track.eta() && scoutingparticle.trk_phi() + scoutingparticle.phi() == track.phi()){
		    reco::TrackRef trackRef(trackHandle, itrack);
    		    pfcand.setTrackRef(trackRef);
	    }
	}
	else{
	    if (scoutingparticle.trk_pt() == track.pt() && scoutingparticle.trk_eta() == track.eta() && scoutingparticle.trk_phi() == track.phi()){
		    reco::TrackRef trackRef(trackHandle, itrack);
    		    pfcand.setTrackRef(trackRef);
	    }
...
void Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::produce(edm::Event &iEvent, edm::EventSetup const &setup) {
  using namespace edm;
  ...
  iEvent.getByToken(input_track_token_, trackHandle);

  auto pfcands = std::make_unique<reco::PFCandidateCollection>();

  if (use_softKiller_) {
    createPFCandidatesSK(scoutingparticleHandle, trackHandle, pfcands);
  } else {
    createPFCandidates(scoutingparticleHandle, trackHandle, pfcands);
  }
...
void Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::clearVars() {
  vertexIndex_.clear();
  normchi2_.clear();
  dz_.clear();
  dxy_.clear();
...
void Run3ScoutingParticleToRecoPFCandidateWithTrackProducer::fillDescriptions(edm::ConfigurationDescriptions &descriptions) {
...
DEFINE_FWK_MODULE(Run3ScoutingParticleToRecoPFCandidateWithTrackProducer);
