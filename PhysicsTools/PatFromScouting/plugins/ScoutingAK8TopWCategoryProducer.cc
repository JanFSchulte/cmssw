// Categorizes AK8 jets into gen-level top/W merging categories, following
// slide 5 of https://cds.cern.ch/record/2941747/files/DP2025_052.pdf:
//
...
//   3 Non-merged: neither of the above (top/W background present, but not
//     merged into this jet).
//   4 Others: processes with no relevant top/W truth chain at all (W+jets,
//     diboson). This is a process-level flag set by the user per sample
//     (applyTopWMerging=False), not something derived from an absence of
//     gen tops in a given event.
//
...
// recorded as a (w1, w2) candidate with no associated b (case 2b above).
...
public:
  ...
  static void fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
    ...
    desc.add<bool>("applyTopWMerging", true)
        ->setComment(
            "set to False for processes with no relevant top/W truth chain (W+jets, VV); every jet is then "
            "categorized as Others without touching genParticles");
  ...
  void produce(edm::Event& iEvent, const edm::EventSetup&) override {
    ...
    if (applyTopWMerging_) {
      ...
      for (std::size_t i = 0; i < jets->size(); ++i)
        categories[i] = categorize(jets->at(i), topHadronicWs, standaloneHadronicWs);
    }
