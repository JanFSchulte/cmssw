...
//
// Package:    V0Producer
// Class:      V0FitterScouting
//
/**\class V0FitterScouting V0Fitter.cc RecoVertex/V0Producer/src/V0Fitter.cc
...
V0FitterScouting::V0FitterScouting(const edm::ParameterSet& theParameters, edm::ConsumesCollector&& iC) : esTokenMF_(iC.esConsumes()) {
  token_vertices = iC.consumes<std::vector<reco::Vertex>>(theParameters.getParameter<edm::InputTag>("vertices"));

  token_tracks = iC.consumes<reco::TrackCollection>(theParameters.getParameter<edm::InputTag>("trackRecoAlgorithm"));

  nValidPixelHitsValueMapToken_ = iC.consumes<edm::ValueMap<int> >(theParameters.getParameter<edm::InputTag>("valueMapNValidPixelHits"));
  nTrackerLayersWithMeasurementValueMapToken_ = iC.consumes<edm::ValueMap<int> >(theParameters.getParameter<edm::InputTag>("valueMapNTrackerLayersWithMeasurement"));
  nValidStripHitsValueMapToken_ = iC.consumes<edm::ValueMap<int> >(theParameters.getParameter<edm::InputTag>("valueMapNValidStripHits"));
...
void V0FitterScouting::fitAll(const edm::Event& iEvent,
  ...
  edm::Handle<edm::ValueMap<int> > nTrackerLayersWithMeasurementValueMap;
  edm::Handle<edm::ValueMap<int> > nValidStripHitsValueMap;

  iEvent.getByToken(nValidPixelHitsValueMapToken_, nValidPixelHitsValueMap);
  iEvent.getByToken(nTrackerLayersWithMeasurementValueMapToken_, nTrackerLayersWithMeasurementValueMap);
  iEvent.getByToken(nValidStripHitsValueMapToken_, nValidStripHitsValueMap);
  const edm::ValueMap<int>& nValidPixelHitsMap = *nValidPixelHitsValueMap;
  const edm::ValueMap<int>& nTrackerLayersWithMeasurementMap = *nTrackerLayersWithMeasurementValueMap;
  ...
  for (reco::TrackCollection::const_iterator iTk = theTrackCollection->begin(); iTk != theTrackCollection->end();
    ...
    reco::TrackRef trackRef(theTrackHandle, std::distance(theTrackCollection->begin(), iTk));
    ...
    int totalHits = nValidPixelHitsMap[trackRef] + nValidStripHitsMap[trackRef]; 
    if (tmpTrack->normalizedChi2() < tkChi2Cut_ && totalHits >= tkNHitsCut_ &&
        tmpTrack->pt() > tkPtCut_ && ipsigXY > tkIPSigXYCut_ && ipsigZ > tkIPSigZCut_) {
      theTrackRefs.push_back(std::move(trackRef));
      reco::TransientTrack tmpTransient(*trackRef, theMagneticField);
  ...
  for (unsigned int trdx1 = 0; trdx1 < theTrackRefs.size(); ++trdx1) {
    for (unsigned int trdx2 = trdx1 + 1; trdx2 < theTrackRefs.size(); ++trdx2) {
      ...
      SMatrixSym3D totalCov =  theVtx.covariance();
