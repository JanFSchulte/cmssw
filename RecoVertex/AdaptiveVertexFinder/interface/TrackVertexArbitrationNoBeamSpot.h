#ifndef TrackVertexArbitrationNoBeamSpot_H
#define TrackVertexArbitrationNoBeamSpot_H
#include <memory>
#include <set>
...
namespace svhelper {
  inline double cov33NoBS(const reco::Vertex &sv) { return sv.covariance(3, 3); }
  inline double cov33NoBS(const reco::VertexCompositePtrCandidate &sv) { return sv.vertexCovariance(3, 3); }
}  // namespace svhelper

template <class VTX>
class TrackVertexArbitrationNoBeamSpot {
public:
  TrackVertexArbitrationNoBeamSpot(const edm::ParameterSet &params);

  std::vector<VTX> trackVertexArbitratorNoBeamSpot(const reco::Vertex &pv,
...
private:
  bool trackFilterArbitratorNoBeamSpot(const reco::TransientTrack &track, int n_pixel_hits, int n_tracker_layers) const;
...
template <class VTX>
TrackVertexArbitrationNoBeamSpot<VTX>::TrackVertexArbitrationNoBeamSpot(const edm::ParameterSet &params)
...
template <class VTX>
bool TrackVertexArbitrationNoBeamSpot<VTX>::trackFilterArbitratorNoBeamSpot(const reco::TransientTrack &track, int n_pixel_hits, int n_tracker_layers) const {
  if (!track.isValid())
    return false;
  if (n_tracker_layers < trackMinLayers)
    return false;
  if (track.track().pt() < trackMinPt)
    return false;
  if (n_pixel_hits < trackMinPixels)
    return false;

  return true;
}

inline float trackWeightNoBS(const reco::Vertex &sv, const reco::TransientTrack &track) {
  return sv.trackWeight(track.trackBaseRef());
}
inline float trackWeightNoBS(const reco::VertexCompositePtrCandidate &sv, const reco::TransientTrack &tt) {
...
template <class VTX>
std::vector<VTX> TrackVertexArbitrationNoBeamSpot<VTX>::trackVertexArbitratorNoBeamSpot(const reco::Vertex &pv,
                                                                    std::vector<reco::TransientTrack> &selectedTracks,
								    std::vector<int> n_pixel_hits,
								    std::vector<int> n_tracker_layers,
                                                                    std::vector<VTX> &secondaryVertices) {
  ...
  for (typename std::vector<VTX>::const_iterator sv = secondaryVertices.begin(); sv != secondaryVertices.end(); ++sv) {
    const double svTime(sv->t()), svTimeCov(svhelper::cov33NoBS(*sv));
    ...
    for (unsigned int itrack = 0; itrack < selectedTracks.size(); itrack++) {
      ...
      if (!trackFilterArbitratorNoBeamSpot(tt, n_pixel_hits[itrack], n_tracker_layers[itrack]))
        continue;
      float w = trackWeightNoBS(*sv, tt);
      Measurement1D ipv;
