      ...
      l2MuonGetToken_(consumes<reco::TrackCollection>(iConfig.getParameter<edm::InputTag>("srcL2")))
{
  ...
  tree->Branch("l2_eta", &t.l2_pt, "l2_eta[100]/F");
  tree->Branch("l2_phi", &t.l2_pt, "l2_phi[100]/F");
  ...
  tree->Branch("segment_globalY", &t.segment_globalX, "segment_globalY[100]/F");
  tree->Branch("segment_globalZ", &t.segment_globalX, "segment_globalZ[100]/F");
   ...
   for (reco::TrackCollection::const_iterator itL2 = l2Muons.begin(); itL2 != l2Muons.end(); itL2++) {
	...
	double previousPhi = -999;
		...
		if (signalSegment){
			...
			t.segment_layerID[nFound] = layerID;
			...
			if (!( previousPhi == -999)){
				t.segment_deltaPhi[nFound-1] = previousPhi = gp.phi().value();
