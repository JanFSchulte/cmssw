// -*- C++ -*-
//
// Package:    RecoMuon/SegmentNtupletAnalyzer
// Class:      SegmentNtupletAnalyzer
//
/**\class SegmentNtupletAnalyzer SegmentNtupletAnalyzer.cc RecoMuon/SegmentAnalyzer/plugins/SegmentNtupletAnalyzer.cc
...
class SegmentNtupletAnalyzer : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
  explicit SegmentNtupletAnalyzer(const edm::ParameterSet&);
  ~SegmentNtupletAnalyzer();
...
private:
  ...
  edm::EDGetTokenT<MuonSegmentNtupletsHeterogeneous> l2MuonGPUGetToken_;
...
SegmentNtupletAnalyzer::SegmentNtupletAnalyzer(const edm::ParameterSet& iConfig)
    : dtSegmentsGetToken_(consumes<DTRecSegment4DCollection>(iConfig.getParameter<edm::InputTag>("srcDT"))),
      ...
      l2MuonGPUGetToken_(consumes<MuonSegmentNtupletsHeterogeneous>(iConfig.getParameter<edm::InputTag>("srcL2sGPU")))
//      theTrackerRecHitBuilderName(iConfig.getParameter<std::string>("TrackerRecHitBuilder"))
{
  ...
  purityNum1   = fs->make<TH1F>("purityNum1", "purityNum1",  23, 0.5,  23.5);
  purityNum2   = fs->make<TH1F>("purityNum2", "purityNum2",  23, 0.5,  23.5);
  purityNum3   = fs->make<TH1F>("purityNum3", "purityNum3",  23, 0.5,  23.5);
  purityDenom = fs->make<TH1F>("purityDenom", "purityDenom",  23, 0.5,  23.5);


  effNum1   = fs->make<TH1F>("effNum1", "effNum1",  23, 0.5,  23.5);
  effNum2   = fs->make<TH1F>("effNum2", "effNum2",  23, 0.5,  23.5);
  effNum3   = fs->make<TH1F>("effNum3", "effNum3",  23, 0.5,  23.5);
  effDenom = fs->make<TH1F>("effDenom", "effDenom",  23, 0.5,  23.5);

  nNtuplets   = fs->make<TH1D>("nNtuplets", "nNtuplets",  21, -0.5,  20.5);
  nSegmentsInNtuplets = fs->make<TH1D>("nSegmentsInNtuplets", "nSegemtsInNtuplets",  6, -0.5,  5.5);


}
SegmentNtupletAnalyzer::~SegmentNtupletAnalyzer() {
...
void SegmentNtupletAnalyzer::beginJob() {
}
void SegmentNtupletAnalyzer::endJob() {}

int SegmentNtupletAnalyzer::pairID(int layerID1, int layerID2) {


	int binIndex = -99;
	if (layerID1 == 5 && layerID2 == 6) std::cout << "found" << std::endl;
	if (layerID1 == 1 && layerID2 == 2) binIndex = 1;
	if (layerID1 == 1 && layerID2 == 5) binIndex = 2;
	if (layerID1 == 1 && layerID2 == 9) binIndex = 3;
	if (layerID1 == 2 && layerID2 == 3) binIndex = 4;
	if (layerID1 == 2 && layerID2 == 5) binIndex = 5;
	if (layerID1 == 2 && layerID2 == 9) binIndex = 6;
	if (layerID1 == 5 && layerID2 == 6) binIndex = 7;
	if (layerID1 == 9 && layerID2 == 10) binIndex = 8;
	if (layerID1 == 3 && layerID2 == 4) binIndex = 9;
	if (layerID1 == 3 && layerID2 == 5) binIndex = 10;
	if (layerID1 == 3 && layerID2 == 9) binIndex = 11;
	if (layerID1 == 6 && layerID2 == 7) binIndex = 12;
	if (layerID1 == 10 && layerID2 == 11) binIndex = 13;
	if (layerID1 == 7 && layerID2 == 8) binIndex = 14;
	if (layerID1 == 11 && layerID2 == 12) binIndex = 15;
	if (layerID1 == 1 && layerID2 == 3) binIndex = 16;
	if (layerID1 == 2 && layerID2 == 4) binIndex = 17;
	if (layerID1 == 1 && layerID2 == 6) binIndex = 18;
	if (layerID1 == 1 && layerID2 == 10) binIndex = 19;
	if (layerID1 == 5 && layerID2 == 7) binIndex = 20;
	if (layerID1 == 6 && layerID2 == 8) binIndex = 21;
	if (layerID1 == 9 && layerID2 == 11) binIndex = 22;
	if (layerID1 == 10 && layerID2 == 12) binIndex = 23;
...
void SegmentNtupletAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  const MuonSegmentNtupletsHeterogeneous& l2MuonsGPU = iEvent.get(l2MuonGPUGetToken_);
...
//  iSetup.get<TransientRecHitRecord>().get(theTrackerRecHitBuilderName,theTrackerRecHitBuilder);
  ...
  auto l2s = l2MuonsGPU.get();
  nNtuplets->Fill(l2s->nNtuplets);
  for (int i = 0; i < l2s->nNtuplets; i++){
	int nHits = l2s->segmentsInNtuplet[i];
	if (nHits == 0) continue;
	nSegmentsInNtuplets->Fill(nHits);
	GlobalPoint gp1 = GlobalPoint(l2s->gx1[i],l2s->gy1[i],l2s->gz1[i]);
	GlobalPoint gp2 = GlobalPoint(l2s->gx2[i],l2s->gy2[i],l2s->gz2[i]);
	GlobalPoint gp3 = GlobalPoint(l2s->gx3[i],l2s->gy3[i],l2s->gz3[i]);
	GlobalPoint gp4;
	if (nHits == 4) gp4 = GlobalPoint(l2s->gx4[i],l2s->gy4[i],l2s->gz4[i]);

	bool signalSegment1 = false;
	bool signalSegment2 = false;
	bool signalSegment3 = false;
	bool signalSegment4 = false;
	for (reco::TrackCollection::const_iterator itL2 = l2Muons.begin(); itL2 != l2Muons.end(); itL2++) {
		...
		for (auto recHit : (*itL2).recHits()){
			...
			TrajectoryMeasurement::ConstRecHitPointer tthit(muonTransBuilder.build(recHit, globalGeometry));
				//TransientTrackingRecHit::RecHitPointer tthit = theTrackerRecHitBuilder->build(&*recHit);
			if ((tthit->globalPosition() - gp1).mag() < 1e-2) signalSegment1 = true;
			if ((tthit->globalPosition() - gp2).mag() < 1e-2) signalSegment2 = true;
			if ((tthit->globalPosition() - gp3).mag() < 1e-2) signalSegment3 = true;
			if (nHits == 4){
				if ((tthit->globalPosition() - gp4).mag() < 1e-2) signalSegment4 = true;
			}
		}
	}
	int pair = pairID(l2s->layerID1[i],l2s->layerID2[i]);
	purityDenom->Fill(pair);
        if (nHits ==4 && signalSegment1 && signalSegment2 && signalSegment3 && signalSegment4) purityNum3->Fill(pair);
        else if (nHits < 4 && signalSegment1 && signalSegment2 && signalSegment3) purityNum3->Fill(pair);
        if (signalSegment1 || signalSegment2 || signalSegment3 || signalSegment4) purityNum1->Fill(pair);
        if ((signalSegment1 && signalSegment2) || (signalSegment1 && signalSegment3 ) || (signalSegment1 && signalSegment4 ) || (signalSegment2 && signalSegment3 ) || (signalSegment2 && signalSegment4 )|| (signalSegment3 && signalSegment4)) purityNum2->Fill(pair);
  ...
  for (reco::TrackCollection::const_iterator itL2 = l2Muons.begin(); itL2 != l2Muons.end(); itL2++) {
	...
	for (int i = 0; i < l2s->nNtuplets; i++){
		...
		if (nHits ==4 && signalSegment1 && signalSegment2 && signalSegment3 && signalSegment4) effNum3->Fill(pair);
		else if (nHits < 4 && signalSegment1 && signalSegment2 && signalSegment3) effNum3->Fill(pair);
		if (signalSegment1 || signalSegment2 || signalSegment3 || signalSegment4) effNum1->Fill(pair);
		if ((signalSegment1 && signalSegment2) || (signalSegment1 && signalSegment3 ) || (signalSegment1 && signalSegment4 ) || (signalSegment2 && signalSegment3 ) || (signalSegment2 && signalSegment4 )|| (signalSegment3 && signalSegment4)) effNum2->Fill(pair);
...
void SegmentNtupletAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("srcL2sGPU", edm::InputTag("hltL2MuonsSoA"));
  desc.add<std::string>("TrackerRecHitBuilder", "WithTrackAngle");
  descriptions.add("SegmentNtupletAnalyzer",desc);

}

DEFINE_FWK_MODULE(SegmentNtupletAnalyzer);
