#ifndef RecoMuon_L2MuonProducer_plugins_GPUCACellMuon_h
#define RecoMuon_L2MuonProducer_plugins_GPUCACellMuon_h

//
// Author: Jan-Frederik Schulte, Purdue
// Based on GPUCACell by Felice Pantaleo, CERN
//
...
public:
  ...
  using Hits = MuonSegmentsCUDAView;
  ...
  static constexpr auto bad = L2MuonTrack::Quality::bad;

  GPUCACellMuon() = default;
  ...
  __device__ __forceinline__ void init(CellNeighborsVector& cellNeighbors,
    ...
    theInnerZ = hh.gz(innerHitId);
    theInnerR = hh.gz(innerHitId);
  ...
  __device__ __forceinline__ float inner_x(Hits const& hh) const { return hh.gx(theInnerHitId); }
  __device__ __forceinline__ float outer_x(Hits const& hh) const { return hh.gx(theOuterHitId); }
  __device__ __forceinline__ float inner_y(Hits const& hh) const { return hh.gy(theInnerHitId); }
  __device__ __forceinline__ float outer_y(Hits const& hh) const { return hh.gy(theOuterHitId); }
  ...
  __device__ __forceinline__ float outer_z(Hits const& hh) const { return hh.gz(theOuterHitId); }
...
#endif  // RecoMuon_L2MuonProducer_plugins_GPUCACellMuon_h
